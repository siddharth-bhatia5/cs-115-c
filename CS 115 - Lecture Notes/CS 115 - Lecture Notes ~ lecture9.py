...
def list_average(l):
    ...
    if (len(l) > 2):
        ...
        # Set up three lists so we can use map(avg_of_three)
        l1 = l[1:-1]
        l2 = l[:-2]
        l3 = l[2:]

        middle = map(avg_of_three, l1, l2, l3)

        # casts head into a list by using the square brackets
        # in order to concatenate the list
        result = [head] + list(middle) + [last]

    return result

print(list_average([4,1,5,6,7,1,9]))
print(list_average([]))
print(list_average([3]))
print(list_average([5,3]))
print(list_average([5,3,100]))
